...
function renderScores() {
  const rows = ST.miembros.map(m => {
    ...
    // Solo mostrar semanas con información (omitir las que no tienen datos)
    ...
    ).filter(Boolean).join(' ') || '<span style="font-size:10px;color:var(--text-4)">Sin histórico</span>';
    return `<tr>
      ...
      <td>${(() => { const et = etiquetaMCI(m); return `<span class="tag ${et.tc}">${esc(et.texto)}</span>`; })()}
