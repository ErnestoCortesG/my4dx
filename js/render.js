...
function renderTablero() {
  ...
  function mciBloque(num, tit, ws) {
    const rows = ws.map(w => {
      ...
      const inp = ro ? '' : `<div class="wirow">
        <input class="winp" type="number" placeholder="Actualizar…" step="0.01"
          onchange="updWIG('${w.id}',this.value)">
        <span style="font-size:10px;color:#aaa">${w.uni}</span>
        <button class="bok" onclick="updWIG('${w.id}',this.previousElementSibling.previousElementSibling.value)">OK</button>
      </div>`;
  ...
  document.getElementById('twigs').innerHTML =
    mciBloque(1, 'Conservación de agentes', ST.wigs.filter(w => w.mci === 1)) +
    mciBloque(2, 'Recluta de claves',       ST.wigs.filter(w => w.mci === 2));
...
function mciParaIntegrante() {
  const ALL = ['MCI 1 · Conservación', 'MCI 2 · Recluta', 'Soporte 4DX', 'Ambos MCIs'];
  if (!su || su.rol !== 'integrante' || !su.mid) return ALL;
  const m = ST.miembros.find(x => x.id === su.mid);
  if (!m) return ALL;
  const unicos = [...new Set(m.preds.map(p => p.mci))];
  // 'Ambos MCIs' se incluye si el miembro tiene preds en MCI 1 y MCI 2
  const tieneM1 = unicos.includes('MCI 1 · Conservación');
  const tieneM2 = unicos.includes('MCI 2 · Recluta');
  if (tieneM1 && tieneM2) unicos.push('Ambos MCIs');
...
function renderAdmin() {
  ...
  const waHtml = ST.wigs.map(w => `<div class="warow">
    <div class="wanum">${w.mci}</div>
    <div>
      <div class="watit">${w.label}</div>
      <div class="wasub">Inicio: ${w.inicio}${w.uni} → Meta: ${w.meta}${w.uni}</div>
      <div class="wasub">${w.sub || ''}</div>
    </div>
  </div>`).join('');
...
function saveMCI() {
  ...
  ST.wigs.push({ id:'w'+uid(), label:tit, inicio:ini, meta, uni, mci:mciN, sub:desc });
  closeModal('m-mci');
  renderAdmin();
  renderTablero();
  guardar();
